...
import { setupAdminSession, generateUniqueName, generateUniqueCustomerName } from '../helpers';
...
test.describe('Admin Dashboard – Items', () => {
    ...
    test('Crear artículo', async () => {
        const nombre = generateUniqueName('TestItem');
        await adminDashboard.addProduct(nombre, 'Rojo', 'Formal', 'S,M', '99');
        await adminDashboard.expectItemExists(nombre);
    });

    test('Editar y Eliminar artículo', async () => {
        const nombre = generateUniqueName('EditDelete');
...
test.describe('Admin Dashboard – Rentals', () => {
    ...
    test('Aceptar reserva: pendiente → confirmado', async ({ page }) => {
        const customerName = generateUniqueCustomerName('PendApprove');
    ...
    test('Cancelar reserva: pendiente → cancelado', async ({ page }) => {
        const customerName = generateUniqueCustomerName('PendCancel');
